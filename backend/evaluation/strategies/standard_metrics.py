...
class StandardMetricsStrategy(EvaluationStrategy):
    def execute(self):
        ...
        relevant_df = gt_df[gt_df['relevance'] >= settings.RELEVANCE_THRESHOLD]
        ...
        for query in tqdm(unique_queries, desc="Evaluating Queries"):
            ...
            for system_id, system_info in settings.SYSTEMS_TO_EVALUATE.items():
                ranked_results = self.client.get_search_results(system_id, query, settings.EVALUATION_K)
                ranked_ids = [item['article_id'] for item in ranked_results]

                precision, recall, f1 = self.metrics.calculate_precision_recall_f1_at_k(
                    ranked_ids, relevant_ids, settings.EVALUATION_K
                )
                results_data.append({
        ...
        system_names_for_print = [sys_info['name'] for sys_info in settings.SYSTEMS_TO_EVALUATE.values()]
        ...
        self.reporting.plot_grouped_bar_chart(
            ...
            systems_config=settings.SYSTEMS_TO_EVALUATE,
