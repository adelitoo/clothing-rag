...
class AnnotationCreationStrategy(EvaluationStrategy):
    def _get_image_path(self, article_id: int) -> str:
        ...
        return os.path.abspath(self.config.IMAGE_BASE_DIR / padded_id[:3] / f"{padded_id}.jpg")
    ...
    def execute(self):
        ...
        articles_df = pd.read_csv(self.config.COMPLETE_ARTICLES_CSV_PATH, dtype={'article_id': int})

        records = []
        seen_pairs = set()
        for query in tqdm(queries, desc="Pooling results"):
            pooled_ids = set()
            for system_id in self.config.SYSTEMS_TO_EVALUATE.keys():
        ...
        output_path = self.config.ANNOTATION_FILE_OUTPUT
