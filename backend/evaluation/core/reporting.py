...
def plot_performance_heatmap(
    summary_df: pd.DataFrame, title: str, output_filename: str, reports_dir: Path
):
    reports_dir.mkdir(parents=True, exist_ok=True)
...
def plot_grouped_bar_chart(
    ...
    ax = plot_df[system_names].plot(kind="bar", color=colors, figsize=(12, 8), rot=0)
...
def plot_score_distribution(
    ...
    palette = {
        sys_info["name"]: sys_info["color"] for sys_info in systems_config.values()
    }
...
def plot_llm_pie_chart(
    summary_df: pd.DataFrame, title: str, output_filename: str, reports_dir: Path
):
    reports_dir.mkdir(parents=True, exist_ok=True)
    preferences = (
        summary_df["llm_preference"]
